...
Franckys_REPL.demo = {
    reqlist:    [
        ...
        '#(element u $(voyelles))',
        '#(element y $(voyelles))',
        '#(map F( #(element $(_1) $(voyelles))) $(voyelles))',
        '#(map F( #(element $(_1) $(voyelles))) #(.. a z))',
        '#(say)',
        '#(say "== 5 == DEFINITION ET UTILISATION D\'OBJETS FONCTIONNELS")',
        '=( est-une-voyelle  F( #(element $(_1) $(voyelles))) )',
        '=( est-une-consonne F( #(not #(est-une-voyelle $(_1)))) )',
        '=( lettres #(.. a z))',
        ...
        '#(eval "(!(:quote :nospace) #(map   F(#(. @(consonnes-2 $(_1)) ,$(voyelles-2)))   #(.. 1 #(consonnes-2)))) )',
        'M( "(!(:quote :nospace) #(map   F(#(. @(consonnes-2 $(_1)) ,$(voyelles-2)))   #(.. 1 #(consonnes-2)))) )',
        '=(* C L(c g m) V L(a e i o u y) acronymes1 F( M("(!(:quote :nospace)"$(C)#(. $ (V))"))))',
        '#(acronymes1)',
        '=(* C L(c g m) V L(a e i o u y) acronymes2 F( M("(!(:quote :nospace)"$(C),$(V)"))))',
        '#(acronymes2)',
        '#(a b c d)',
        '=("f o o" f o o)',
        '#("f o o")',
        ...
        '#($("+ - * /") 1 2 4 8)',
        '#($("+ - * /") 1 2 4 8)',
        '=(gcd1 F(    #(cmp? $(_1) $(_2) #(gcd1 $(_1) #(- $(_2) $(_1))) $(_1) #(gcd1 #(- $(_1) $(_2)) $(_2)))))',
        '=(gcd2 F(    ?( #(== $(_2) 0) $(_1) #(self $(_2) #(mod $(_1) $(_2))))))',
        '=(gcd225 F( #(gcd2 225 $(_1)) ) )',
        '#(gcd1 225 3)',
        '#(gcd2 225 5)',
        '#(gcd1 225 9)',
        '#(gcd2 225 15)',
        ...
        '=(fact1 F( ?( #(le $(_1) 1) 1 #(* $(_1) #(self #(1- $(_1)))))))',
        '#(fact1 0)',
        '#(fact1 1)',
        '#(fact1 2)',
        '#(fact1 3)',
        ...
        '#(fibit 2)',
        '#(fibit 6)',
        '=(reset-fibit F( #(progn  =(* x 0 y 1) #(fibit $(_1)))))',
        '#(reset-fibit 15)',
        '=(reset-fibit-2 F( P( =(* x 0 y 1) #(fibit $(_1)))))',
        '#(reset-fibit-2 15)',
        '=(* x 0 y 1 fibit2  I( 1($(y) =(* x $(y) y #(+$(x)$(y))))))',
        '=(reset-fibit-3 F( P( =(* x 0 y 1) #(fibit2 $(_1)))))',
        '#(reset-fibit-3 15)',
        '=(fib-fun F( #(== #(progn =(* x 0 y 1) #(+ #(map fibit #(.. 1 $(_1))))) #(progn =(* x 0 y 1) #(+ #(fibit #(+ #(.. 1 $(_1)))))))))',
        '#(map fib-fun #(.. 1 20))',
        '#(map 1 #(.. 1 20))',
        '=(fib-fun-2 F( #(== P( =(* x 0 y 1) #(+ #(map fibit #(.. 1 $(_1))))) P( =(* x 0 y 1) #(+ #(fibit #(+ #(.. 1 $(_1)))))))))',
        '#(map fib-fun-2 #(.. 1 20))',
        '#(reset-fibit 1)',
        '#(reset-fibit 2)',
        '#(reset-fibit 3)',
        ...
        '#(say)',
        '#(say "== 7 == ASCII-ART")',
        '=(ascii-art F( P( =(* x   #(2- $(_1)) I   I(*($(x)" ")N) foo F( #(. #(I $(x))))) #(say ?(#(== $(_1) 1) N "N#(map foo #(.. 1 $(_1)))N")))))',
        '#(ascii-art 1)',
        '#(ascii-art 3)',
        '#(ascii-art 5)',
        '#(ascii-art 7)',
        '#(ascii-art 9)',
        '#(map F( #(ascii-art $(_1))) 1 3 5 7 9)',
        '#(say)',
        '#(say "== 8 == MATRIX OPERATIONS")',
        '=(Matrix V( V( #(.. 111 115) ) V( #(.. 121 125) ) V( #(.. 131 135) ) V( #(.. 141 145) )) V( V( #(.. 211 215) ) V( #(.. 221 225) ) V( #(.. 231 235) ) V( #(.. 241 245) )) V( V( #(.. 311 315) ) V( #(.. 321 325) ) V( #(.. 331 335) ) V( #(.. 341 345) )))',
        '@(!(:matrix-mode) Matrix 1 * *)',
        '@(!(:matrix-mode) Matrix 2 * *)',
        '@(!(:matrix-mode) Matrix 3 * *)',
        '@(!(:matrix-mode) Matrix * 1 *)',
        '@(!(:matrix-mode) Matrix * 2 *)',
        '@(!(:matrix-mode) Matrix * 3 *)',
        '@(!(:matrix-mode) Matrix * 4 *)',
        '@(!(:matrix-mode) Matrix * * 1)',
        '@(!(:matrix-mode) Matrix * * 2)',
        '@(!(:matrix-mode) Matrix * * 3)',
        '@(!(:matrix-mode) Matrix * * 4)',
        '@(!(:matrix-mode) Matrix * * 5)',
        '@(!(:matrix-mode) Matrix * * *)',
        '#(# @(!(:matrix-mode) Matrix * * *))',
        '#(+ @(!(:matrix-mode) Matrix * * *))',
        '@(!(:matrix-mode) Matrix L(1 2) L(3 4) 5)',
        '@(!(:matrix-mode) Matrix L(3 2 1) L(4 3 2 1) L(5 4 3 2 1))',
        '@(!(:matrix-mode) Matrix #(.. 3 1) #(.. 4 1) #(.. 5 1))',
        '#(say)',
        ...
        '=(calcul-du-prix F( #(+ $(prix-de-base) #(assoc S(_1) $(supplément-prix)))))',
        '=(INVENTAIRE-MUTINY #(map  F("$(_1) => #(calcul-du-prix $(_1)) CFP") "Couleur:$(couleurs) Taille:$(tailles) Col:$(col)"))',
        '#(say)',
        '#(say "EXEMPLE 2")',
        '#(say)',
        '=(Prix_Article F( #(+ #(assoc* $(_1) MODEL=1 1000 MODEL=2 2000 MODEL=3 3000 XTRA=1 100 XTRA=2 200 XTRA=3 300 XTRA=4 400 XTRA=5 500 TH=3 30 TH=4 40 TH=5 50 CT=1 1 CT=3 3 CT=4 4 CT=5 5 CT=6 6 * NONE))))',
        '=(Description_Article F("(#(assoc* $(_1) MODEL=1 "Tshirt MUTINY Vintage; " MODEL=2 "Tshort Kalachnikov; " MODEL=3 "Tshirt Vespa; " XTRA=1 "Capuche; " XTRA=2 "Ouverture zippée; " XTRA=3 "Ouverture boutons; " XTRA=4 "Maille piquée; " XTRA=5 "Bretelles; " TH=3 "Homme Taille médium; " TH=4 "Home Taille large; " TH=5 "Homme grande taille; " CT=1 "Coloris Heather Tan" CT=3 "Coloris Heather Navy" CT=4 "Coloris Bleu" CT=5 "Coloris Asphalte" CT=6 "Coloris Métal" * "???"))))',
        '=(Créer_Article F( "#(Description_Article $(_1)). Prix: #(Prix_Article $(_1)) CFP." ))',
        ...
        '#(say "EXEMPLE 3")',
        '#(say)',
        'P(=(* L "A(MODEL= Coconut Asphalte Peace-and-Love) A(SIZE= S M L XL) A(COLOR= red blue yellow green)" LA L(MODEL "Modèle: " SIZE "Taille: " COLOR "Couleur: ") mcont F("#(assoc $(_1) $(LA)) $(_2)") ccont F("( !( :join-with ",, ") $(@)))) ~( !(:c-continuation ccont :m-continuation mcont) L (MODEL,)=* (SIZE,)=* (COLOR,)=* ) )',
        '#(say)',
