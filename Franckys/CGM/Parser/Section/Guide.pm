#===============================================================================
#
#         FILE: Franckys/CGM/Parser/Section/Guide.pm
#
#        USAGE: <+USAGE+>
#
...
#===============================================================================
package Franckys::CGM::Parser::Section::Guide;
...
# my $bool                  = $section->is_mandatory_field($index);
# my $bool                  = $section->is_template_field($index);
...
sub dump {
    ...
    # SECTION HEADER
    #
    say sprintf('<h1>Section <i>%s</i></h1>', $self->name() );
    ...
    my ($field_index, $record, $field, @field_values, $header, $in_ul_section );

    foreach $record ( $self->records() ) {
#say("[GUIDE] new record : ", scalar @$record);
        $field_index = 0;

        foreach $field (@$record) {
            $header = $headers[$field_index]; 
            @field_values = map { s|\R+|<br />|gs; $_ } $self->get_record_field_values( $record, $field_index );
#say("[GUIDE] fieldindex:$field_index header:$header fieldValues:", scalar @field_values);

            if ($header && @field_values) {
                if ( $header eq 'li' ) {
                    unless ( $in_ul_section ) {
                        say '<ul>';
                        $in_ul_section = 1;
                    }
                }
...
=head1 Franckys::CGM::Parser::Section::Guide
