...
sub validate_record {
    ...
    my ($varname) = $self->get_record_field_values($record, $VARNAME_FIELDINDEX);
    my @values    = $self->get_record_field_values($record, $VARVALUE_FIELDINDEX);
...
sub record_as_html {
    ...
    my ($varname) = ( $section->get_record_field_values($record, $VARNAME_FIELDINDEX) )[0];
    my @values    = map { "\"$_\"" }  $section->get_record_field_values($record, $VARVALUE_FIELDINDEX);
